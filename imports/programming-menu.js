...
export class ProgrammingMenu {
  ...
  this.menuItems.map((item, index) => {
    this.processMenuItem(item, index);
  });
  
  this.renderedElement.instance = this;
  }

  async processMenuItem(menuItem, index) {
    let x = ((index % this.itemsPerRow) * this.margin) + this.initX;
    let y = (Math.floor(index / this.itemsPerRow) * this.margin) + this.initY;

    await menuItem.render(this.renderedElement);
    menuItem.renderedElement.setAttribute('position', {x: x, y: y, z: this.initZ});
  }
