...
/***/ function(module, exports) {
	...
	AFRAME.registerComponent('resizable', {
	  ...
	  tick: function tick() {
	    ...
	    var handPos = new THREE.Vector3().copy(this.resizers[0].getAttribute('position')),
	        otherHandPos = new THREE.Vector3().copy(this.resizers[1].getAttribute('position')),
	        currentStretch = handPos.distanceTo(otherHandPos),
	        deltaStretch = 1;
	    if (this.previousStretch !== null && currentStretch !== 0) {
	      deltaStretch = currentStretch / this.previousStretch;
	  ...
	  start: function start(evt) {
	    ...
	    if (this.resizers.indexOf(evt.detail.hand) === -1) {
	      return;
	    }
	    this.resizers = [];
	    this.resized = false;
